...
namespace R54IN0.Test
{
    ...
    public class InventoryEditorViewModelTest
    {
        ...
        public void CopyInventoryPipe()
        {
            ...
            using (var db = DatabaseDirector.GetDbInstance())
            {
                var inven = db.LoadAll<Inventory>().FirstOrDefault();
                ...
                Assert.AreEqual(inven.SpecificationUUID, pipe.Specification.UUID);
            }
        }
        ...
        public void AllSpecificationTest()
        {
            ...
            using (var db = DatabaseDirector.GetDbInstance())
            {
                for (int i = 0; i < 20; i++)
                {
                    ...
                    var spec1 = db.LoadByKey<Specification>(viewModel.SelectedSpecification.UUID);
                    var spec2 = db.LoadByKey<Specification>(inven.SpecificationUUID);

                    Assert.AreEqual(spec1.Name, spec2.Name);
                    Assert.AreEqual(viewModel.SelectedSpecification.UUID, inven.SpecificationUUID);
                    Assert.AreEqual(viewModel.SelectedWarehouse.UUID, inven.WarehouseUUID);

                    var uuid = viewModel.AllSpecification.FirstOrDefault().UUID;
                }
            }
