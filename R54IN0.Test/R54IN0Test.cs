...
namespace R54IN0.Test
{
    ...
    public class R54IN0Test
    {
        ...
        public void InOutStockTest()
        {
            ...
            //재고관리헬퍼로 재고관리 생성
            var cur_stock = new InventoryRRRHelper().Save(spec, ware, 10, remark_str); //10개 등록
            ...
            //현재 재고 물량과 일치하는지 확인
            cur_stock = DatabaseDirector.GetDbInstance().LoadByKey<CurrentStock>(cur_stock.UUID);
            Assert.AreEqual(cur_stock.SpecificationUUID, spec.UUID);
            Assert.AreEqual(cur_stock.TraceItem().UUID, item.UUID);
            Assert.AreEqual(cur_stock.ItemCount, 30);

            //출고 관리 데이터 추가 -15
            InOutStock out_stock = new InOutStockRRRHelper().Save(StockType.OUT, DateTime.Now, spec, 15, seller, eep, ware, remark_str);
